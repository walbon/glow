...
template <typename T>
static void RescaleBias(Node &node, Function *F, int factor) {
  ...
  // There is a trick here. When rescaleBiasOpt flag is set, the bias is
  // rescaled 1 q-point below result for conv/FC with fused activations.
  auto scale = resTy->getScale() * factor;
...
NMPBackend::transformPostOptPipeline(Function *F,
  ...
  for (auto &node : F->getNodes()) {
    ...
    case Kinded::Kind::ConvolutionNodeKind: {
      ...
      int cnFactor{1};
      if (rescaleBiasOpt)
        cnFactor = CN->getFusedActivation() == FusedActivation::NONE ? 1 : 2;
      RescaleBias<ConvolutionNode>(node, F, cnFactor);
      break;
    }
    ...
    case Kinded::Kind::NMPFullyConnectedNodeKind: {
      ...
      int fcFactor{1};
      if (rescaleBiasOpt)
        fcFactor = FC->getFusedActivation() == FusedActivation::NONE ? 1 : 2;
      RescaleBias<NMPFullyConnectedNode>(node, F, fcFactor);
      break;
    }

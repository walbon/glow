...
void NMPLLVMIRGen::generatePoolInstr(llvm::IRBuilder<> &builder,
  ...
  auto *scale_ddr_base = LLVMIRGen::emitConstI32(builder, 0);
  ...
  if (srcTy->getScale() == destTy->getScale()) {
    // There's an issue on Inception 3 due to RescaleQuantizedInst
    // that I can't figure out. So the factor below is a palliative
    // solution until I find out how to fix it.
    auto name = PI->getName();
    if (name == "InceptionV3_InceptionV3_Mixed_7a_Branch_2_MaxPool_1a_3x3_MaxPool__1")
      factor = 1;
  }
  ...
  createCall(builder, F,
             {dfxpMode,      srcPtr,        destPtr,    scale_ddr_base,
...
int32_t NMPLLVMIRGen::generateIRCodeForInstr(llvm::IRBuilder<> &builder,
  ...
  case Kinded::Kind::ConvolutionInstKind: {
    ...
    if (!(padBottom == 0 && padLeft == 0 && padTop == 0 && padRight == 0))
    ...
    int factor{0};
    if (rescaleBiasOpt)
      factor = CI->getFusedActivation() == FusedActivation::NONE ? 0 : 1;
    ...
    assert(biasTy->getScale() == destTy->getScale() * (factor + 1) &&
           "Bias scale not match with internal Convolution scale.");
    int16_t srcQValue = log2(1 / srcTy->getScale());
    int16_t filterQValue = log2(1 / filterTy->getScale());
    int16_t destILScale = srcQValue - log2(1 / destTy->getScale()) + factor;
    ...
    auto *actILScale = LLVMIRGen::emitConstI16(builder, -factor);
    ...
    if (isdw) {
      createCall(builder, F,
                 {dfxpMode,        srcPtr,        destPtr,       filterPtr,
                  ...
                  act_type,        srcQP,         filterQP,      outILScale,
                  actILScale,      srcMode,       _num_tle,      _num_tlt,
                  _thread_np_y,    _slice_ofm_w,  _slice_ofm_h,  _slice_filters,
                  _ofm_ch_per_tlt, _mb_size});
    } else {
      createCall(builder, F, {dfxpMode,     srcPtr,        destPtr,
                              filterPtr,    biasPtr,       inWidth,
                              inHeight,     inChannels,    outWidth,
                              outHeight,    conv_kernel_w, conv_kernel_h,
                              filterNum,    conv_pad_w,    conv_pad_h,
                              strides,      dilation,      act_type,
                              srcQP,        filterQP,      outILScale,
                              actILScale,   srcMode,       _num_tlt,
                              _thread_np_y, _thread_np_c,  _thread_conv_wgts_mb,
                              _slice_ofm_w, _slice_ofm_h,  _slice_ifm_c,
                              _mb_size});
    }
    return 1;
  }
  ...
  case Kinded::Kind::ReluInstKind: {
    ...
    createCall(builder, F,
               {out_dtype, srcPtr, destPtr, in_w, actv, in_fxp, alpha_fxp,
                out_ls, dtype_cast, part_thread_w, part_slice_w, ntlts});
    return 1;
  }
  ...
  case Kinded::Kind::NMPFullyConnectedInstKind: {
    ...
    int factor{0};
    if (rescaleBiasOpt)
      factor = FCI->getFusedActivation() == FusedActivation::NONE ? 0 : 1;
    ...
    auto *biasTy = bias->getType();
    assert(biasTy->getScale() == destTy->getScale() * (factor + 1) &&
           "Bias scale not match with NMPFullyConnected result scale.");
    int16_t srcQValue = log2(1 / srcTy->getScale());
    ...
    createCall(builder, F,
               {dfxpMode, srcPtr, weightsPtr, biasPtr, destPtr, dest_m, dest_n,
                gemm_k, act_type, srcQP, weightsQP, outILScale, actILScale,

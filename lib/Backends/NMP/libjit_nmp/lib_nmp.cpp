...
{
  return nmp_activation_layer(out_dtype, in_base, out_base, in_w, activation,
                              in_fxp, alpha_fxp, out_ls, dtypecast,
                              part_thread_w, part_slice_w, ntlts_per_tle);
}
...
int _nmp_conv_layer_istn_ofm(
    ...
    int16_t act_q_ls,        // Quantize Param (activation layer ls)
    ...
    uint16_t mapper_cnfg_mb_size)      // Mapper's config memblob size of tlt
{
  return nmp_conv_layer_istn_ofm(
      ...
      conv_q_df, conv_q_wf, conv_q_ls, act_q_ls, conv_q_loadmode,
...
int _nmp_conv_layer_istn_wgt(
    ...
    uint16_t act_type,            // Activation type
    uint16_t neg_slope,           // neg slope param
    int16_t conv_q_df,            // Quantize Param (conv_df)
    int16_t conv_q_wf,            // Quantize Param (conv_wf)
    int16_t conv_q_ls,            // Quantize Param (conv_ls)
    int16_t act_q_ls,             // Quantize Param (activation layer ls)
    ...
    uint16_t mapper_cnfg_mb_size)      // Mapper's config memblob size of tlt
{
  return nmp_conv_layer_istn_wgt(
      ...
      neg_slope, conv_q_df, conv_q_wf, conv_q_ls, act_q_ls, conv_q_loadmode,
    ...
    uint16_t mapper_cnfg_mb_size)        // memblob size of tlt
{
  return nmp_conv_layer_wstn_ofm_dw(
      ...
      conv_pad_w, conv_pad_h, conv_stride, conv_dilation, act_type, conv_q_df,
      conv_q_wf, conv_q_ls, act_q_ls, conv_q_loadmode, mapper_cnfg_num_tle,
...
int _nmp_pool_layer(
    ...
    uint32_t scale_ddr_base,   // Layer Output(XXX) base address
    ...
    uint16_t mapper_cnfg_slice_ofm_h) // Mapper's config sliding window height
{
  return nmp_pool_layer(
      dfxp_mode, ifm_ddr_base, ofm_ddr_base, scale_ddr_base, ofm_ddr_base_idx, ifm_w, ifm_h,
                      ...
                      uint16_t num_tlt)
{
  return nmp_matmul_layer(dfxp_mode, lhs_addr, rhs_addr, bias_addr, out_addr,
                          m, n, k, act_type, dot_q_df,
                          dot_q_wf, dot_q_ls, act_q_ls, matmul_q_loadmode,
                          num_iters, num_rows, num_rows_remaining, num_tle,
                          num_tlt);
}
